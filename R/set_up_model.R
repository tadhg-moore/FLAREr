...
set_up_model <- function(config,
  ...
  if(config$model_settings$model == "glm_aed"){
    ...
    file.copy(from = file.path(config$file_path$configuration_directory, "forecast_model", config$model_settings$model, config$model_settings$base_AED_nml),
              to = file.path(ens_working_directory, "aed2.nml"), overwrite = TRUE)

    file.copy(from = file.path(config$file_path$configuration_directory, "forecast_model", config$model_settings$model, config$model_settings$base_AED_phyto_pars_nml),
              to = file.path(ens_working_directory, "aed2_phyto_pars.nml"), overwrite = TRUE)

    file.copy(from = file.path(config$file_path$configuration_directory, "forecast_model", config$model_settings$model, config$model_settings$base_AED_zoop_pars_nml),
