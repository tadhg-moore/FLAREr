...
#'
#' Read in model restart variables in a list format. From `LakeEnsemblR`
#'
#'@param folder folder
...
read_restart <- function(folder = ".", model) {
  ...
  if(model == "GLM") {
    ...
    nml_file <- file.path(folder, "glm3.nml") # adjusted from LER
    if(!file.exists(nml_file)) {
      stop("No 'glm3.nml' file in ", folder)
    }
    nml <- read_nml(nml_file)
    out_dir <- get_nml_value(nml, "out_dir")
    the_depths <- get_nml_value(nml, "the_depths")

    outfile <- file.path(folder, out_dir, "output.nc") # adjusted from LER
